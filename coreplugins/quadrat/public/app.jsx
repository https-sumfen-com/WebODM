...
export default class App {
  constructor(map, injectedTask) {
    ...
    this.apiBase = config.API_BASE || window.QUADRAT_API_BASE || 'http://localhost:7700'
...
map.on('measurepopupshown', ({ popupContainer, model, resultFeature }) => {
      ...
      const existingRaw = resultFeature._quadratProps && resultFeature._quadratProps.raw
      ...
      if (isNewQuadrat) {
        ...
        this.promptQuadratName((name) => {
          ...
          if (!resultFeature._quadratProps.raw) resultFeature._quadratProps.raw = {}
          resultFeature._quadratProps.raw.name = name
          
          // 渲染弹窗
          this.renderQuadratPopup(popupContainer, model, resultFeature, foundLayer)
          this.renderQuadratList()
          // 仅新样方才提交统计
          this.generateAllQuadratStats()
        })
  ...
  renderQuadratPopup(popupContainer, model, resultFeature, layer) {
    ...
    ReactDOM.render(<QuadratPopup
      ...
      map={this.map}
      onEditGeometry={() => this.startGeometryEdit(layer, resultFeature)} />, $container.get(0))
  }
  ...
  showEditControls() {
    ...
    $controls.find('.btn-confirm').on('click', () => this.confirmGeometryEdit())
  ...
  async confirmGeometryEdit() {
    ...
    if (resultFeature && resultFeature._quadratProps) {
      resultFeature._quadratProps.vertices = newCoords.map(c => ({ lon: c[0], lat: c[1] }))
      // 重新计算中心点
  ...
  async updateQuadratGeometry() {
    ...
    try {
      const response = await fetch(`${this.apiBase}/api/odm/samplings/${sid}/statistics`, {
        method: 'POST',
  ...
  renderQuadratList() {
    ...
    this.map.eachLayer(layer => {
      ...
      if (mp && mp.state && mp.state.featureType === 'Polygon') {
        ...
        if (coords.length) {
          ...
          const raw = (mp && mp.props && mp.props.resultFeature && mp.props.resultFeature._quadratProps && mp.props.resultFeature._quadratProps.raw) || null
          const id = raw ? raw.id : null
          const name = raw && raw.name ? raw.name : null
    ...
    if (items.length > 0) {
      ...
      items.forEach((it, idx) => {
        const $li = $('<li class="quadrat-item" data-layer-id="' + it.layer._leaflet_id + '"/>')
        // 优先使用 name 字段，回退到"样方 N"
        const displayName = it.name || `${_('样方')} ${idx + 1}`
        
        // 名称容器（显示模式）
        const $nameWrapper = $('<span class="quadrat-name-wrapper"/>')
        const $nameText = $(`<span class="quadrat-name-text" title="${displayName}">${displayName}</span>`)
        const $nameInput = $(`<input type="text" class="quadrat-name-edit-input" value="${displayName}" style="display:none;"/>`)
        ...
        const $edit = $("<span href='#' class='edit' title='编辑名称'><i class='fa fa-edit'></i></span>")
        const $confirm = $("<span href='#' class='confirm' title='确认' style='display:none;'><i class='fa fa-check'></i></span>")
    ...
    this.map.eachLayer(layer => {
      ...
      if (mp && mp.state && mp.state.featureType === 'Polygon') {
        ...
        if (vertices.length) {
          ...
          const raw = (mp && mp.props && mp.props.resultFeature && mp.props.resultFeature._quadratProps && mp.props.resultFeature._quadratProps.raw) || {}
          const name = raw.name || ''
          items.push({ layer, payload: { idx, coords: vertices, center: [cx, cy], name } })
        }
      }
    })
    return items
  ...
  generateAllQuadratStats() {
    ...
    if (this._isGeneratingStats) {
      console.log('[Quadrat] Stats generation already in progress, skipping...')
      return
    }
    ...
    this.getSamplingId().then(samplingId => {
      ...
      $.ajax({ type: 'POST', url: `${this.apiBase}/api/odm/samplings/${samplingId}/statistics`, data: JSON.stringify(payload), contentType: 'application/json' })
           ...
           this.pollTimer = setInterval(() => {
             ...
             $.ajax({ type: 'GET', url: `${this.apiBase}/api/odm/samplings/${samplingId}` })
               ...
               .fail(err => { 
                 ...
                 this.pollTimer = null
                 this._isGeneratingStats = false
                 alert(_('样方状态查询失败: ') + (err && err.status ? err.status : _('网络错误')))
                 this.removeUnsavedQuadrats()
                 this.renderQuadratList()
               })
           }, 2000)
        ...
        .fail(err => { 
          const errorMsg = err && err.status ? (_('统计任务启动失败: ') + err.status) : _('统计任务启动失败')
          this.setAllPopupsError(errorMsg)
          alert(_('样方分析提交失败: ') + (err && err.status ? err.status : _('网络错误')))
          this._isGeneratingStats = false
          this.removeUnsavedQuadrats()
          this.renderQuadratList()
        })
  ...
  addQuadratLayer(q){
    ...
    poly.on('click', (e) => {
      ...
      ReactDOM.render(<QuadratPopup model={model} resultFeature={resultFeature} map={this.map} onEditGeometry={() => this.startGeometryEdit(poly, resultFeature)} />, container)
    })
  }
  ...
  async updateQuadratName(it, newName){
    ...
    if (mp && mp.props && mp.props.resultFeature && mp.props.resultFeature._quadratProps) {
